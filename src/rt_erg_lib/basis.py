...
class Basis(object):
    ...
    def __init__(self, explr_space, num_basis=5, offset=None):
        if offset is not None:
            raise NotImplementedError('ERROR: We have not implemented offsets yet!')
        ...
        ## self.hk was not being used -- does it make a difference? (2022/2/1)
    ...
    def fk(self, x):
        assert (x.shape[0] == self.dl.shape[0]), 'input dimension does not match explr dimension'

        ## if not using hk normalization 
        return np.prod(np.cos(np.pi*x/self.dl * self.k),1)#/self.hk

        ## if using hk normalization
        #return np.prod(np.cos(np.pi*x/self.dl * self.k),1)/self.hk
